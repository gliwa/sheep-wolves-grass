...
// the tick rate (the server applies ≤1 per entity per tick).
...
// press time (Shift = wolf), releasing the arrow releases the move.
...
// Re-send held moves so holding a key means continuous movement (SPEC.md →
// Movement); slightly faster than the default tick so no tick goes empty.
setInterval(() => {
  if (!isOnPlayScreen(state)) {
    held.clear();
    return;
  }
  for (const move of held.values()) send({ type: 'move', ...move });
}, MOVE_REPEAT_MS);
